#include "mainwindow.h"
#include "ui_mainwindow.h"
#include<QFileDialog>
#include<QDateTime>

MainWindow::MainWindow(QWidget *parent)
    : QMainWindow(parent)
    , ui(new Ui::MainWindow)
{
    ui->setupUi(this);

    //header
    ui->treeWidget->clear();
    QTreeWidgetItem *header = new QTreeWidgetItem();
    header->setText(colItem,"目录和文件");
    header->setText(colItemType,"节点类型");
    header->setText(colDate,"修改时间");
    header->setTextAlignment(colItemType,Qt::AlignHCenter|Qt::AlignVCenter);

    ui->treeWidget->setHeaderItem(header);

    //添加根节点
    QIcon icon(":/images/icon/wenjianjia.png");
    QTreeWidgetItem *topItem = new QTreeWidgetItem(itTopItem);
    topItem->setIcon(colItem,icon);
    topItem->setText(colItem,"图片");
    topItem->setText(colItemType,"top Item");
    topItem->setFlags(Qt::ItemIsSelectable|Qt::ItemIsEnabled|Qt::ItemIsDropEnabled|Qt::ItemIsUserCheckable);
    topItem->setCheckState(colItem,Qt::Checked);
    ui->treeWidget->addTopLevelItem(topItem);

    labelNodeText = new QLabel("节点标题",this);
    labelNodeText->setMidLineWidth(300);
    ui->toolBar_statue->addWidget(labelNodeText);

    spinRatio = new QSpinBox(this);
    spinRatio->setRange(0,2000);
    spinRatio->setValue(100);
    spinRatio->setSuffix(" %");
    spinRatio->setReadOnly(true);
    spinRatio->setButtonSymbols(QAbstractSpinBox::NoButtons);
    ui->toolBar_statue->addWidget(spinRatio);

    labelfileName =new  QLabel ("文件名",this);
    ui->toolBar->addSeparator();
    ui->toolBar_statue->addWidget(labelfileName);

}

MainWindow::~MainWindow()
{
    delete ui;
}

void MainWindow::on_actaddFolder_triggered()
{
    QString dir = QFileDialog::getExistingDirectory();
    if(dir.isEmpty()) return;
    QTreeWidgetItem *parentItem = ui->treeWidget->currentItem();
    if(parentItem==nullptr) return;
    if(parentItem->type()!=itImageItem){
        int cnt = dir.length();
        int i =dir.lastIndexOf("/");
        QString dirname = dir.right(cnt-i-1);
        QTreeWidgetItem *item = new QTreeWidgetItem(itGroupItem);
        QIcon icon(":/images/icon/wenjianjia.png");
        item->setIcon(colItem,icon);
        item->setText(colItem,dirname);
        item->setText(colItemType,"Group Item");
        item->setFlags(Qt::ItemIsSelectable|Qt::ItemIsEnabled|Qt::ItemIsDropEnabled|Qt::ItemIsUserCheckable);
        item->setCheckState(colItem,Qt::Checked);
        item->setData(colItem,Qt::UserRole,QVariant(dir));
        parentItem->addChild(item);
    }
    parentItem->setExpanded(true);
}


void MainWindow::on_actaddfile_triggered()
{
    QStringList files = QFileDialog::getOpenFileNames(this,"选择文件","","Images(*.jpg)");
    if(files.isEmpty()) return;

    QTreeWidgetItem *parItem,*item;
    item=ui->treeWidget->currentItem();
    if(item==nullptr) item=ui->treeWidget->topLevelItem(0);
    if(item->type()==itImageItem)
        parItem = item->parent();
    else
        parItem = item;
    for(int i=0;i<files.size();i++){
        QString fileName = files.at(i);
        QIcon icon(":/images/icon/tupian.png");
        QFileInfo fileInfo(fileName);
        QString NodeText = fileInfo.fileName();
        QDateTime dateTime = fileInfo.lastModified();
        QTreeWidgetItem *item = new QTreeWidgetItem(itImageItem);
        item->setIcon(colItem,icon);
        item->setText(colItem,NodeText);
        item->setText(colItemType,"Image Item");
        item->setText(colDate,dateTime.toString("yyyy-MM-dd"));
        item->setData(colItem,Qt::UserRole,QVariant(fileName));
        parItem->addChild(item);
    }
    parItem->setExpanded(true);
}


void MainWindow::on_actdelfile_triggered()
{
    QTreeWidgetItem *item =ui->treeWidget->currentItem();
    if(item==nullptr) return;
    QTreeWidgetItem *parItem = item->parent();
    parItem->removeChild(item);
}


void MainWindow::on_treeWidget_currentItemChanged(QTreeWidgetItem *current, QTreeWidgetItem *previous)
{
    if(current==nullptr||current==previous) return;
    switch (current->type()) {
    case itTopItem:
        ui->actaddFolder->setEnabled(true);
        ui->actaddfile->setEnabled(true);
        ui->actdelfile->setEnabled(false);
        break;
    case itGroupItem:
        ui->actaddFolder->setEnabled(true);
        ui->actaddfile->setEnabled(true);
        ui->actdelfile->setEnabled(true);
        break;
    case itImageItem:
        ui->actaddFolder->setEnabled(false);
        ui->actaddfile->setEnabled(true);
        ui->actdelfile->setEnabled(true);
        displayImage(current);
        break;
    default:
        break;
    }

}


void MainWindow::on_actshowall_triggered()
{
    ui->labelpix->setPixmap(m_pixmap);
    m_ratio =1;
    spinRatio->setValue(100);
}

void MainWindow::changeItemCaption(QTreeWidgetItem *item)
{
    QString str = "*"+item->text(colItem);
    item->setText(colItem,str);
    if(item->childCount()>0){
        for(int i=0;i<item->childCount();i++){
            changeItemCaption(item->child(i));
        }
    }
}


void MainWindow::on_actzoomrealsize_triggered()
{
    int w = ui->scrollArea->width()-30;
    int realw = m_pixmap.width();
    m_ratio = float(w)/realw;
    spinRatio->setValue(m_ratio*100);

    QPixmap pix = m_pixmap.scaledToWidth(w-30);
    ui->labelpix->setPixmap(pix);
}


void MainWindow::on_actzoombyw_triggered()
{
    int w = ui->scrollArea->width()-30;
    int realw = m_pixmap.width();
    m_ratio = float(w)/realw;
    spinRatio->setValue(m_ratio*100);

    QPixmap pix = m_pixmap.scaledToWidth(w-30);
    ui->labelpix->setPixmap(pix);
}


void MainWindow::on_actzoombyh_triggered()
{
    int h = ui->scrollArea->height()-30;
    int realh = m_pixmap.height();
    m_ratio = float(h)/realh;
    spinRatio->setValue(m_ratio*100);

    QPixmap pix = m_pixmap.scaledToHeight(h-30);
    ui->labelpix->setPixmap(pix);
}

void MainWindow::displayImage(QTreeWidgetItem *item)
{
    QString filename = item->data(colItem,Qt::UserRole).toString();
    labelfileName->setText(filename);
    labelNodeText->setText(item->text(colItem));
    m_pixmap.load(filename);
    ui->actzoombyh->triggered();
    ui->actzoombyh->setEnabled(true);
    ui->actzoombyw->setEnabled(true);
    ui->actZoomIn->setEnabled(true);
    ui->actzoomOut->setEnabled(true);
}


void MainWindow::on_actZoomIn_triggered()
{
    m_ratio*=1.2;
    int w = m_ratio*m_pixmap.width();
    int h =m_ratio*m_pixmap.height();
    ui->labelpix->setPixmap(m_pixmap.scaled(w,h));
    spinRatio->setValue(100*m_ratio);
}


void MainWindow::on_actzoomOut_triggered()
{
    m_ratio*=0.8;
    int w = m_ratio*m_pixmap.width();
    int h =m_ratio*m_pixmap.height();
    ui->labelpix->setPixmap(m_pixmap.scaled(w,h));
    spinRatio->setValue(100*m_ratio);
}





void MainWindow::on_actshow_triggered(bool checked)
{
    ui->dockWidget->setVisible(checked);
}


void MainWindow::on_actflow_triggered(bool checked)
{
    ui->dockWidget->setFloating(checked);
}


void MainWindow::on_dockWidget_visibilityChanged(bool visible)
{
    ui->actshow->setChecked(visible);
}


void MainWindow::on_dockWidget_topLevelChanged(bool topLevel)
{
    ui->actflow->setChecked(topLevel);
}

